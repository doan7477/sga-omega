...
bool pixelCollisionManager::isPixelCollisionLeftX(image* destImage, float destX, float destY, image* sourImage)	
{
	...
	for (int i = _probeX - 20; i < _probeX; ++i)
	{
...
bool pixelCollisionManager::isPixelCollisionBombTopY(image* destImage, float destX, float destY, image* sourImage)
{
	_probeY = destY;
...
bool pixelCollisionManager::isPixelCollisionBombBottomY(image* destImage, float destX, float destY, image* sourImage)
{
	_probeY = destY;

	for (int i = _probeY; i < _probeY + destImage->getFrameHeight() / 2; ++i)
	{
		_color = GetPixel(sourImage->getMemDC(), destX, i);
...
bool pixelCollisionManager::isPixelCollisionBombLeftX(image* destImage, float destX, float destY, image* sourImage)
{
	_probeX = destX;
...
bool pixelCollisionManager::isPixelCollisionBombRightX(image* destImage, float destX, float destY, image* sourImage)
{
	_probeX = destX;

	for (int i = _probeX; i < _probeX + destImage->getFrameWidth() / 2 + 50; ++i)
	{
		_color = GetPixel(sourImage->getMemDC(), i, destY);
...
bool pixelCollisionManager::testPixelCollision(image* destImg, float destX, float destY, image* sourImg, int range, int r, int g, int b)
{
	_probeY = destY + destImg->getFrameHeight() / 2;
